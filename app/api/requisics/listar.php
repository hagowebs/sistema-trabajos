<?php // app/api/requisics/listar.php
...
try {
    ...
    $query = "SELECT id, requisic, nota, fecha_inicial, fecha_final, estado FROM requisics WHERE 1=1";
    $params = [];
    
    // Aplicar la regla de ocultar 'entregados' si no hay filtros activos
    if ($ocultar_entregados) {
        $query .= " AND estado != 'archivado'";
    }

    // 1. Filtro por rango de fechas
    if (!empty($fecha_inicio) && !empty($fecha_fin)) {
        $query .= " AND DATE(fecha_inicial) BETWEEN :fecha_inicio AND :fecha_fin";
        $params['fecha_inicio'] = $fecha_inicio;
        $params['fecha_fin'] = $fecha_fin;
    } elseif (!empty($fecha_inicio)) {
        $query .= " AND DATE(fecha_inicial) >= :fecha_inicio";
        $params['fecha_inicio'] = $fecha_inicio;
    } elseif (!empty($fecha_fin)) {
        $query .= " AND DATE(fecha_inicial) <= :fecha_fin";
        $params['fecha_fin'] = $fecha_fin;
    }
    ...
    $stmt->execute($params);
    $requisics = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $response = [
        "data" => $requisics
    ];
